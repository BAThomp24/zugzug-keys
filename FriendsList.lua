...
----------------------------------------------------------------------
-- Debug dump — exposed via /zzk friendsdebug
----------------------------------------------------------------------

function Keys.DumpFriendsListDebug()
  print("|cff8fbf3fZugZug Keys:|r friends list debug")
  print("  FriendsListFrame=" .. tostring(_G.FriendsListFrame)
    .. "  FriendsFrame=" .. tostring(_G.FriendsFrame)
    .. "  ContactsFrame=" .. tostring(_G.ContactsFrame))
  local sb, container = findFriendsScrollBox()
  print("  resolved scrollbox=" .. tostring(sb) .. "  container=" .. tostring(container))
  if not sb then
    print("  |cffFF6666no scrollbox found — friends list internals unknown|r")
    return
  end
  local n, matched = 0, 0
  sb:ForEachFrame(function(btn, elementData)
    n = n + 1
    if n <= 5 then
      local data = (btn.GetElementData and btn:GetElementData()) or elementData
      print(string.format("  [%d] btn=%s elemData=%s GetElementData=%s",
        n, tostring(btn), tostring(elementData), tostring(data)))
      ...
      local info = accountInfoForButton(btn)
      print("       accountInfo=" .. tostring(info))
      if info then
        print("       customMessage=" .. tostring(info.customMessage))
        print("       clientProgram=" .. tostring(info.gameAccountInfo and info.gameAccountInfo.clientProgram))
        if info.customMessage then
          local p = parseBroadcast(info.customMessage)
          print("       parsed=" .. tostring(p) .. (p and string.format(" (+%d %s ~%s)", p.level, p.dungeon, p.finish) or ""))
          if p then matched = matched + 1 end
        end
      end
    end
  end)
  print(string.format("  total visible buttons=%d, broadcast-matched=%d", n, matched))
end
